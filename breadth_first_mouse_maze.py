...
class tree:
		...
		if len(path)%2 == 0:
			...
			for move in node.reachable:
				...
				if not new in self.explored:
					...
					self.frontier_paths.add((new,tuple(new_path)))
		else:
	...
	def search_frontier(self):
		new_fronts = copy.deepcopy(self.frontier_paths)
		for p, path in new_fronts:
			self.expand_node(maze(self.n, self.m, p[0], p[1]), path)
		self.frontier_paths = self.frontier_paths - new_fronts
